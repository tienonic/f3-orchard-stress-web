...
const SITES = [
  { id: 'partner_site_1', name: 'Partner Site 1', note: 'recommended first, clearest orchard-scale view' },
  { id: 'tulare_site_1', name: 'Tulare Site 1', note: 'readable public-boundary example' },
  { id: 'fresno_site_1', name: 'Fresno Site 1', note: 'larger public-boundary example' },
  { id: 'kern_site_1', name: 'Kern Site 1', note: 'small low-priority check' },
  { id: 'kings_site_1', name: 'Kings Site 1', note: 'large public-boundary example' },
  { id: 'stanislaus_site_1', name: 'Stanislaus Site 1', note: 'largest public-boundary example' }
];
...
const coordinateInstruction = isTouchFirst
  ? 'Tap or drag the map for coordinates, then tap a colored zone to inspect it.'
  : 'Hover for coordinates, select a zone, then launch Google Maps to scout the chosen point.';
const coordinateIdleText = isTouchFirst ? 'Tap map' : 'Move over map';
const LIVE_APP_URL = 'https://f3-orchard-stress-web.vercel.app';
const PUBLIC_GITHUB_URL = 'https://github.com/tienonic/f3-orchard-stress-web';

// The page is rendered from data files so the app can stay static and easy to host.
...
app.innerHTML = `
  <header class="shell header">
    <div>
      ...
      <p class="lede">
        Open a site, ${coordinateInstruction}
        The layer prioritizes field follow-up; it does not diagnose disease, irrigation failure, soil, nutrients, or yield loss.
        The site picker starts with the clearest review examples before the larger public-boundary maps.
      </p>
    </div>
    <div class="header-actions" aria-label="Project links">
      <p class="header-note">Public field demo: site selection, coordinates, zone status, and Google Maps routing.</p>
      <div class="header-links">
        <a class="live-link" href="${LIVE_APP_URL}" target="_blank" rel="noreferrer">Live app</a>
        <a class="github-link" href="${PUBLIC_GITHUB_URL}" target="_blank" rel="noreferrer">Public GitHub</a>
      </div>
    </div>
  </header>
  ...
  <main class="shell page-grid">
    <section class="quickstart" aria-labelledby="quickstart-title">
      ...
      <ol class="steps">
        <li><span>1</span><p>Start with Partner Site 1, then use the selector to move through the larger examples.</p></li>
        ...
        <li><span>3</span><p>${isTouchFirst ? 'Tap or drag anywhere on the map' : 'Hover anywhere on the map'} to read the latitude and longitude.</p></li>
        <li><span>4</span><p>${isTouchFirst ? 'Tap' : 'Click'} a colored zone or exact point of interest to revise the selected coordinate before routing.</p></li>
        ...
        <li class="map-cue"><p>Scroll down to see the map</p></li>
      </ol>
    </section>
      ...
      <div class="map-layout">
        <div class="map-wrap">
          ...
          <div class="coordinate-readout">
            <span>Cursor</span>
            <strong id="cursorCoords">${coordinateIdleText}</strong>
          </div>
        </div>
        ...
        <aside class="detail-panel" aria-live="polite">
          ...
          <div id="zoneDetails" class="zone-details">
            <p class="empty-state">${isTouchFirst ? 'Tap' : 'Click'} a colored zone or a top work-order row to select a field coordinate.</p>
          </div>
        </aside>
      </div>
    </section>
...
// Populate the site picker from the same IDs used by the static GeoJSON files.
SITES.forEach((site) => {
...
async function init() {
  // CSV tables drive the summary cards and the ranked field-stop table.
...
async function loadSite(siteId) {
  // Swap only the active layer and selection state when changing sites.
  clearSelected();
...
function selectZone(zone, layer = null, selectedLatLng = null) {
  ...
  const routeCoordinate = selectedLatLng
    ? { lat: Number(selectedLatLng.lat), lon: Number(selectedLatLng.lng), exactPoint: true }
    : { lat: Number(zone.lat), lon: Number(zone.lon), exactPoint: false };

  // Reuse one marker so repeated clicks stay fast and visually clean.
  state.selectedCoordinate = routeCoordinate;
  if (state.selectedMarker) {
    state.selectedMarker.removeFrom(map);
  }

  state.selectedMarker = L.circleMarker([routeCoordinate.lat, routeCoordinate.lon], {
    radius: 6,
    color: '#111827',
    fillColor: '#ffffff',
    fillOpacity: 1,
  ...
  renderZoneDetails(zone, routeCoordinate);
}
...
function selectPoint(latlng) {
  ...
  state.selectedCoordinate = { lat: Number(latlng.lat), lon: Number(latlng.lng), exactPoint: true };
  if (state.selectedMarker) {
    state.selectedMarker.removeFrom(map);
  }

  state.selectedMarker = L.circleMarker([latlng.lat, latlng.lng], {
    radius: 6,
    color: '#0f766e',
    fillColor: '#ffffff',
    fillOpacity: 1,
  ...
  renderPointDetails(state.selectedCoordinate);
}
...
function clearSelected() {
  ...
  zoneDetails.innerHTML = `<p class="empty-state">${isTouchFirst ? 'Tap' : 'Click'} a colored zone or a top work-order row to select a field coordinate.</p>`;
}

function renderZoneDetails(zone, routeCoordinate = { lat: Number(zone.lat), lon: Number(zone.lon), exactPoint: false }) {
  const meta = CLASS_META[zone.zoneClass] || CLASS_META['Stable context'];
  const mapsUrl = googleMapsUrl(routeCoordinate.lat, routeCoordinate.lon);
  const pointNote = routeCoordinate.exactPoint
    ? 'Route opens the exact point you selected inside this zone. Use the zone classification as context, then adjust based on legal access, rows, and field conditions.'
    : zone.recommendation || meta.action;
  zoneDetails.innerHTML = `
    ...
    <dl>
      ...
      <div><dt>Selected coordinate</dt><dd>${formatCoord(routeCoordinate.lat, routeCoordinate.lon)}</dd></div>
      ${routeCoordinate.exactPoint ? `<div><dt>Zone centroid</dt><dd>${formatCoord(zone.lat, zone.lon)}</dd></div>` : ''}
...
function renderPointDetails(routeCoordinate) {
  const mapsUrl = googleMapsUrl(routeCoordinate.lat, routeCoordinate.lon);
  zoneDetails.innerHTML = `
    <div class="status-pill" style="--zone-color:#0f766e">
      <span></span>Custom navigation point
    </div>
    <h3>Selected point of interest</h3>
    <dl>
      <div><dt>Problematic?</dt><dd>Not classified by itself. Use nearby zone color, canopy condition, and field context.</dd></div>
      <div><dt>Selected coordinate</dt><dd>${formatCoord(routeCoordinate.lat, routeCoordinate.lon)}</dd></div>
    </dl>
    <p>${isTouchFirst ? 'Tap again' : 'Click again'} on the map to revise this point before opening Google Maps.</p>
    <div class="route-card">
      <span>Open this exact point in Google Maps?</span>
      <a href="${mapsUrl}" target="_blank" rel="noreferrer">Yes, open route</a>
    </div>
  `;
}
...
function updateWorkOrders(siteId) {
  ...
  workOrderRows.innerHTML = rows.map((row) => `
    <tr>
      <td data-label="Rank">${escapeHtml(row.work_order_rank)}</td>
      <td data-label="Zone"><button class="zone-button" type="button" data-zone-id="${escapeHtml(row.zone_id)}">${escapeHtml(row.zone_id)}</button></td>
      <td data-label="Class"><span class="class-dot" style="--zone-color:${CLASS_META[row.priority_class]?.color || '#64748b'}"></span>${escapeHtml(row.priority_class)}</td>
      <td data-label="Acres">${formatNumber(row.approx_area_acres, 2)}</td>
      <td data-label="Persistence">${formatNumber(row.persistence_score, 2)}</td>
      <td data-label="Coordinate">${formatCoord(row.centroid_lat, row.centroid_lon)}</td>
      <td data-label="Route"><a class="text-link" href="${googleMapsUrl(row.centroid_lat, row.centroid_lon)}" target="_blank" rel="noreferrer">Google Maps</a></td>
    </tr>
  `).join('');
...
function parseCsv(text) {
  // Small CSV parser for the static data files; supports quoted cells.
  const rows = [];
  let row = [];
  let cell = '';
  let inQuotes = false;
